...
func TestCmdBuild_WithTaskAndModel(t *testing.T) {
	...
	assertSequence(t, cmd, "--model", "claude-opus-4-6")
}
...
func TestCmdBuild_SkillsSymlinks(t *testing.T) {
	...
	for _, name := range []string{"skill-a", "skill-b"} {
		link := filepath.Join(wsDir, ".claude", "skills", name)
	...
	notADir := filepath.Join(wsDir, ".claude", "skills", "not-a-dir.txt")
